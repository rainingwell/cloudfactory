...
<div class="layuimini-container">
    <div class="layuimini-main">
        ...
        <script type="text/html" id="currentTableBar">
            {{# if(d.orderstatus == "已发布"){ }}
            <a class="layui-btn layui-btn-xs" lay-event="toubiao">投标详情</a>
            {{# } }}
            {{# if(d.orderstatus == "已发货"){ }}
            <a class="layui-btn layui-btn-xs layui-bg-orange " lay-event="shouhuo">收货</a>
            {{# } }}
            {{# if(d.orderstatus == "已保存"){ }}
            <a class="layui-btn layui-btn-xs layui-btn-danger data-count-delete" lay-event="delete">删除</a>
            <a class="layui-btn layui-btn-normal layui-btn-xs data-count-edit" lay-event="edit">编辑</a>
            <a class="layui-btn layui-btn-xs " lay-event="fabu">发布</a>
            {{# } }}
            {{# if(d.orderstatus == "订单完成"){ }}
            <a class="layui-btn layui-btn-xs layui-btn-danger data-count-delete" lay-event="delete">删除</a>
            {{# } }}
        </script>
...
<script>
    layui.use(['form', 'table'], function () {
        ...
        table.render({
            elem: '#currentTableId',
            method: 'post',
            url: '${pageContext.request.contextPath}/order/list2',
            toolbar: '#toolbarDemo',
        ...
        table.on('toolbar(currentTableFilter)', function (obj) {
            if (obj.event === 'add') {  // 监听添加操作
                var index = layer.open({
                    ...
                    shadeClose: true,
                    area: ['100%', '100%'],
                    content: '../statics/page/order/addorder.html',
        ...
        table.on('tool(currentTableFilter)', function (obj) {
            ...
            if (obj.event === 'edit') {
                ...
                var index = layer.open({
                    ...
                    shadeClose: true,
                    area: ['100%', '100%'],
                    content: '../statics/page/order/edit.html',
                    success: function(layero, index){
                        var body = layer.getChildFrame('body', index);
                        var iframeWin = window[layero.find('iframe')[0]['name']]; //得到iframe页的窗口对象，执行iframe页的方法：iframeWin.method();
                        console.log(body.html()); //得到iframe页的body内容
                        ...
                        // body.find ('#productno').val(obj.data.productno);
                    }
                });
                $(window).on("resize", function () {
                    layer.full(index);
            ...
            }else if (obj.event === 'fabu') {
                layer.confirm('确定发布订单么', function (index) {
                    var option="2";
                    $.ajax({
                        type:"GET",
                        url:'${pageContext.request.contextPath}/order/updatestatus',
                ...
                var index = layer.open({
                    ...
                    shadeClose: true,
                    area: ['100%', '100%'],
                    content: '../statics/page/order/bid.html',
                    success: function(layero, index){
                        var body = layer.getChildFrame('body', index);
                        var iframeWin = window[layero.find('iframe')[0]['name']]; //得到iframe页的窗口对象，执行iframe页的方法：iframeWin.method();
                        console.log(body.html()); //得到iframe页的body内容
